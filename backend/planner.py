...
def _explicit_step_height(product_type: str, step_name: str) -> int:
    product = _norm_text(product_type)
    text = _norm_text(step_name)
    if any(k in text for k in ["modal", "dialog", "drawer", "popover", "tooltip", "toast", "popup"]):
        return 960
    if any(k in text for k in [
        "home", "landing", "about", "pricing", "article", "blog", "report", "documentation",
        "docs", "knowledge", "help", "support", "catalog", "category", "product", "checkout",
        "cart", "payment", "billing", "form", "search results"
    ]):
        return 2200
    if any(k in product for k in [
        "landing", "ecommerce", "marketplace", "documentation", "article", "report",
        "support", "knowledge", "content", "media", "education", "travel"
    ]):
        return 2200
    return 1080
...
def _build_plan_from_layout_context(
    ...
    feature_group = layout_type if layout_type and layout_type != "State" else "Main Experience"
    ...
    sections = [{
        "section_name": "Main Content",
        "purpose": layout_description or "Recreate the supplied interface as a full-frame experience with the same structural intent.",
