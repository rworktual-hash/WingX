...
LAYOUT_ANALYSIS_PROMPT = """You are a senior UI/UX analyst. You are examining a visual reference (screenshot, mockup, design image, app screen, website, portal, form, report, document page, or mixed interface).

Extract ONLY the visual and structural information — do NOT invent content.

Return JSON:
{
  "layout_type": "A short generic label for the interface archetype, such as landing_page, product_screen, dashboard, ecommerce, form, article, docs, portal, marketplace, editor, analytics, report, profile, auth, settings, search_results, media_gallery, support_center, or other",
  ...
  "screen_type": "A generic structural scope such as full_page, long_scroll_page, app_screen, modal, drawer, panel, component, document_page, slide, sheet, report, or partial",
...
Extract ONLY the content, features, structure, and interactions described. Do NOT invent visual details.
The document may describe a website, app, admin tool, marketplace, report, article, portal, workflow system, knowledge base, or any other domain.

Return JSON:
{
  "product_type": "A short generic label for the content/domain archetype, such as crm, project_management, ecommerce, dashboard, saas, landing_page, documentation, report, education, healthcare, finance, travel, media, marketplace, support_center, profile_portal, article, search, workflow_tool, or other",
...
Output JSON:
{
  ...
  "design_type": "A short generic design label such as product_screen, landing_page, dashboard, ecommerce, documentation, portal, report, article, marketplace, form_flow, auth_flow, or other",
...
def _merge_layout_analyses(analyses: list[dict]) -> dict:
    ...
    merged = {
        "layout_type":         _pick_dominant_string(analyses, "layout_type", fallback="other"),
        "visual_style":        " | ".join(a["analysis"].get("visual_style", "") for a in analyses if a["analysis"].get("visual_style")),
        "detected_sections":   _merge_unique_strings(a["analysis"].get("detected_sections", []) for a in analyses),
        "detected_components": _merge_unique_strings(a["analysis"].get("detected_components", []) for a in analyses),
        "layout_description":  " ".join(a["analysis"].get("layout_description", "") for a in analyses),
        "color_palette":       " | ".join(a["analysis"].get("color_palette", "") for a in analyses if a["analysis"].get("color_palette")),
        "screen_type":         _pick_dominant_string(analyses, "screen_type", fallback="full_page"),
...
def _merge_content_analyses(analyses: list[dict]) -> dict:
    ...
    merged = {
        "product_type":     _pick_dominant_string(analyses, "product_type", fallback="other"),
        "features":         _merge_unique_strings(a["analysis"].get("features", []) for a in analyses),
        "interactions":     _merge_unique_strings(a["analysis"].get("interactions", []) for a in analyses),
        "entities":         _merge_unique_strings(a["analysis"].get("entities", []) for a in analyses),
        "pages_or_screens": _merge_unique_strings(a["analysis"].get("pages_or_screens", []) for a in analyses),
        "content_summary":  " ".join(a["analysis"].get("content_summary", "") for a in analyses),
        "key_workflows":    _merge_unique_strings(a["analysis"].get("key_workflows", []) for a in analyses),
...
def _pick_dominant_string(analyses: list[dict], field: str, fallback: str = "") -> str:
    counts: dict[str, tuple[int, str]] = {}
    for entry in analyses:
        value = str(entry.get("analysis", {}).get(field, "") or "").strip()
        if not value:
            continue
        key = value.lower()
        score, original = counts.get(key, (0, value))
        counts[key] = (score + 1, original)

    if not counts:
        return fallback

    winner = sorted(counts.values(), key=lambda item: (-item[0], len(item[1])))[0][1]
